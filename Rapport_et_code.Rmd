...
MHWeibull_REffect <- function(iter = 10^4,
  ...
  for (i in 1:iter){
    ...
    for (j in 1:6){
      ...
      top <- dnorm(prop[j],0,100, log = TRUE) + sum(cens1*dweibull(time1,shape=shape,scale=scale1, log=TRUE)+cens2*dweibull(time2,shape=shape,scale=scale2, log=TRUE)+(1-cens1)*pweibull(time1,shape=shape,scale=scale1, log=TRUE, lower.tail = FALSE)+(1-cens2)*pweibull(time2,shape=shape,scale=scale2, log=TRUE, lower.tail = FALSE))
      ...
      bottom <- dnorm(current[j],0,100, log = TRUE) + sum(cens1*dweibull(time1,shape=shape,scale=scale1, log=TRUE)+cens2*dweibull(time2,shape=shape,scale=scale2, log=TRUE)+(1-cens1)*pweibull(time1,shape=shape,scale=scale1, log=TRUE, lower.tail = FALSE)+(1-cens2)*pweibull(time2,shape=shape,scale=scale2, log=TRUE, lower.tail = FALSE))
    ...
    current[7] <- rinvgamma(1,shape=10^(-3)+38/2, scale=10^(-3)+sum(current[9:46]^2)/2)
  
    
    #Modfifying r
    #Proposition
    prop[8]<-rnorm(1,current[8],prop.sd[7])
    ...
    top <- dgamma(prop[8],1,10^(3), log = TRUE) + sum(cens1*dweibull(time1,shape=shape,scale=scale1, log=TRUE)+cens2*dweibull(time2,shape=shape,scale=scale2, log=TRUE)+(1-cens1)*pweibull(time1,shape=shape,scale=scale1, log=TRUE, lower.tail = FALSE)+(1-cens2)*pweibull(time2,shape=shape,scale=scale2, log=TRUE, lower.tail = FALSE))
    ...
    bottom <- dgamma(current[8],shape=1,rate=10^3, log = TRUE) + sum(cens1*dweibull(time1,shape=shape,scale=scale1, log=TRUE)+cens2*dweibull(time2,shape=shape,scale=scale2, log=TRUE)+(1-cens1)*pweibull(time1,shape=shape,scale=scale1, log=TRUE, lower.tail = FALSE)+(1-cens2)*pweibull(time2,shape=shape,scale=scale2, log=TRUE, lower.tail = FALSE))
    ...
    for (j in 9:46){
      ...
      top <- dnorm(prop[j],0,100, log = TRUE) + sum(cens1[j-8]*dweibull(time1[j-8],shape=shape,scale=scale1, log=TRUE)+cens2[j-8]*dweibull(time2[j-8],shape=shape,scale=scale2, log=TRUE)+(1-cens1[j-8])*pweibull(time1[j-8],shape=shape,scale=scale1, log=TRUE, lower.tail = FALSE)+(1-cens2[j-8])*pweibull(time2[j-8],shape=shape,scale=scale2, log=TRUE, lower.tail = FALSE))
      ...
      bottom <- dnorm(prop[j],0,100, log = TRUE) +  sum(cens1[j-8]*dweibull(time1[j-8],shape=shape,scale=scale1, log=TRUE)+cens2[j-8]*dweibull(time2[j-8],shape=shape,scale=scale2, log=TRUE)+(1-cens1[j-8])*pweibull(time1[j-8],shape=shape,scale=scale1, log=TRUE, lower.tail = FALSE)+(1-cens2[j-8])*pweibull(time2[j-8],shape=shape,scale=scale2, log=TRUE, lower.tail = FALSE))
